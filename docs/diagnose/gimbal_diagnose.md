...
| action | 作用 | 关键判定 | 示例 |
| --- | --- | --- | --- |
| `quick` | 发包+收包快速检查（3s） | `bytes>0` 且 `ok28/ok49>0` | `./diagnostics/gimbal/diagnose.sh quick` |
| `rxonly` | 只收包检查（不发控制） | `ok28/ok49>0` | `./diagnostics/gimbal/diagnose.sh rxonly` |
...
- `bytes>0` 且 `ok28=0 ok49=0`：有字节流，但不是目标协议帧。
- `last_chunk` 长期全 `00`：通常是端口对了但数据源异常（电控发送侧未形成有效帧，或读到非协议通道）。
- `ok49>0` 或 `ok28>0`：协议已对上，链路可用。
- `proto` 失败：当前端口/固件状态下没有有效回传帧。

## 5. 相关说明

- 非英雄协议默认为：`0xA5` 下发，`0x5A` 回传（兼容 28B/49B 回传帧）。
- 若需调英雄链路，请使用 `hero_gimbal_ui_test` / `usb_aim_rx_test`，不要和当前链路混用。
