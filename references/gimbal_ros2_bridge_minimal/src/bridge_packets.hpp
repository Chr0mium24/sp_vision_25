#ifndef GIMBAL_ROS2_BRIDGE_MINIMAL__BRIDGE_PACKETS_HPP
#define GIMBAL_ROS2_BRIDGE_MINIMAL__BRIDGE_PACKETS_HPP

#include <array>
#include <cstdint>
#include <cstring>

namespace minimal_bridge
{
...
{
  uint8_t header = kGimbalToVisionHeader;
...
{
  uint8_t header = kVisionToGimbalHeader;
  uint8_t tracking = 0;
  float pitch = 0.0f;
  float yaw = 0.0f;
  uint8_t fire = 0;
...
static_assert(sizeof(GimbalToVision) == 49, "Unexpected GimbalToVision size");
static_assert(sizeof(VisionToGimbal) == 14, "Unexpected VisionToGimbal size");
...
inline uint16_t get_crc16(const uint8_t * data, uint32_t len)
{
  ...
  while (len--) {
    ...
    const uint8_t i = (crc16 ^ byte) & 0x00ff;
...
{
  return get_crc16(reinterpret_cast<const uint8_t *>(&packet), sizeof(T) - sizeof(uint16_t));
}
