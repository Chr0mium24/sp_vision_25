#ifndef CAMERA_THREAD_TEST_SUPPORT_HPP
#define CAMERA_THREAD_TEST_SUPPORT_HPP
  ...
  const std::string & config_path, int number, bool debug)
{
  ...
  for (int i = 0; i < number; i++) {
    yolo11s.emplace_back(config_path, debug);
  }
  return yolo11s;
}
  ...
  const std::string & config_path, int number, bool debug)
{
  ...
  for (int i = 0; i < number; i++) {
    yolov8s.emplace_back(config_path, debug);
  }
  return yolov8s;
}
...
#endif  // CAMERA_THREAD_TEST_SUPPORT_HPP
